...
module.exports.run = async function ({ event, api, Currencies, Users, args }) {
    try {
        ...
        if (!bet || isNaN(bet) || bet < 1000) return HakiraSEND("địt mẹ 1000 cũng éo đặt thì chơi làm cc gì", threadID, messageID);
        if (bet > money) return HakiraSEND("Nghèo mà đòi chơi tài xỉu là tk l", threadID, messageID);
        ...
        var msg = `===== TÀI XỈU =====` +
            ...
            '\n' +
            `[ x ] - Kết Quả của nhà cái: ${ans}` +
            '\n' +
            `[ x ] - Bạn ${name} đã cược ${choose} với số tiền ${replace(bet)} và ${(result === 'win' ? 'bạn đã thắng' : 'bị nhà cái lấy')}: ${replace(Math.floor(mn))}$` +
            '\n' +
            `[ x ] - Số Tiền còn lại trong tài khoản của ${name} là: ${replace(mne)}$` +
            '\n' +
            `===== TÀI XỈU =====`;
