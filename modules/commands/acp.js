...
module.exports.handleReply = async ({ handleReply, event, api }) => {
  ...
  if (args[0] == "add") {
    ...
    return api.sendMessage("Vui lòng chọn <add | del > <số thứ tự | hoặc \"all\">", event.threadID, event.messageID);
  }
  ...
  const resultMessage = success.length > 0 ?
    `» Đã ${args[0] == 'add' ? 'chấp nhận' : 'xóa'} lời mời kết bạn thành công của ${success.length} người:\n${success.join("\n")}` :
    `» Không có lời mời kết bạn nào để ${args[0] == 'add' ? 'chấp nhận' : 'xóa'}.`;
...
module.exports.run = async ({ event, api }) => {
  ...
  if (listRequest.length === 0) {
    return api.sendMessage("Không có lời mời kết bạn.", event.threadID);
  }
